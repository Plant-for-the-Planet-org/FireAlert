...
// Zod schema for GOES-16 provider configuration validation
const GOES16ConfigSchema = z
  .object({
    bbox: z.string(),
    slice: z.string(),
    client: z.literal(GeoEventProviderClient.GOES16),
    privateKey: z.object({
      type: z.string(),
      project_id: z.string(),
...
class GOES16GeoEventProviderClass implements GeoEventProviderClass {
    ...
    return new Promise<GeoEvent[]>(async (resolve, reject) => {
      try {
        ...
        const twoHoursAgo = new Date(
          currentDateTime.getTime() - 2 * 3600 * 1000,
        );
        const fromDateTime =
          !lastRunDate ||
          currentDateTime.getTime() - lastRunDate.getTime() > 2 * 3600 * 1000
            ? twoHoursAgo
            : lastRunDate;

        const images = ee
          .ImageCollection('NOAA/GOES/16/FDCF')
          .filterDate(fromDateTime, currentDateTime);
        const getImagesId = () => {
          return new Promise((resolve, reject) => {
            images.evaluate(imageCollection => {
              if (imageCollection && imageCollection.features) {
                const imagesData = imageCollection.features.map(
                  feature => feature.id,
                );
                resolve(imagesData);
              } else {
            ...
            const fireData = (await new Promise((resolve, reject) => {
              temperatureVector.evaluate(featureCollection => {
                if (featureCollection && featureCollection.features) {
                  ...
                  // [long, lat, eventDate]
                  const fireDataWithTime = featureCollection.features.map(
                    feature => [...feature.geometry.coordinates, datetime],
                  );
        ...
        const geoEventsData: GeoEvent[] = allFireData.map(
          (fireData: FireDataEntry) => ({
            type: 'fire',
            latitude: fireData[1],
  ...
  getConfig(): GOES16GeoEventProviderConfig {
    if (typeof this.config === 'undefined') {
      throw new Error(
        `Invalid or incomplete GOES-16 event provider configuration`,
      );
    }

    try {
      // Use Zod schema for validation
      const validatedConfig = GOES16ConfigSchema.parse(this.config);
      return validatedConfig as GOES16GeoEventProviderConfig;
    } catch (error) {
      if (error instanceof z.ZodError) {
        const fieldErrors = error.errors
          ...
          .join(', ');
        throw new Error(
          `Invalid GOES-16 provider configuration: ${fieldErrors}`,
        );
      }
      throw new Error(
        `Invalid GOES-16 provider configuration: ${
