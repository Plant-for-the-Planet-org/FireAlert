...
import {GeoEventProviderValidator} from '../../core/validators/GeoEventProviderValidator';
...
class NasaGeoEventProviderClass implements GeoEventProviderClass {
  ...
  initialize(config?: GeoEventProviderConfigGeneral): void {
    ...
    const validation = GeoEventProviderValidator.validateConfig(
      config,
      GeoEventProviderValidator.getNasaProviderRules(),
    );
    ...
    return new Promise<GeoEvent[]>(async (resolve, reject) => {
      try {
        ...
        parser
          ...
          .on('error', error => {
            reject(new ProviderDataParsingError(this.getKey(), 'CSV', error));
          });
      } catch (error) {
        reject(new ProviderDataFetchError(this.getKey(), error));
      }
    });
  }
  ...
  getConfig(): NasaGeoEventProviderConfig {
    if (typeof this.config === 'undefined') {
      throw new ProviderConfigurationError(this.getKey(), 'config');
    }
    const config = this.config;
    if (typeof config.apiUrl === 'undefined') {
      throw new ProviderConfigurationError(this.getKey(), 'apiUrl');
    }
    if (typeof config.bbox === 'undefined') {
      throw new ProviderConfigurationError(this.getKey(), 'bbox');
    }
    if (typeof config.slice === 'undefined') {
      throw new ProviderConfigurationError(this.getKey(), 'slice');
    }
    if (typeof config.client === 'undefined') {
      throw new ProviderConfigurationError(this.getKey(), 'client');
    }
