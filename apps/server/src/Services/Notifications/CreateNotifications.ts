...
function createNestedChunksForUnprocessedSiteAlerts(unprocessedSiteAlerts:UnprocessedSiteAlert[], approxChunkSize: number = 50){
  ...
  unprocessedSiteAlerts.forEach((siteAlert) => {
      ...
      const shouldCreateNewChunk = isNewSiteId && currentChunk.length >= approxChunkSize;

      if (shouldCreateNewChunk || isNewSiteId) {
          nestedIndex++;
...
const createNotifications = async () => {
  ...
  try {
    ...
    const siteAlertsInChunks = createNestedChunksForUnprocessedSiteAlerts(unprocessedAlerts, 10)
