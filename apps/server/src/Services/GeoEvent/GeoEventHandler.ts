...
const processGeoEvents = async (
  ...
  const {newGeoEvents, deletedIds} = compareIds(
  ...
  logger(
    `${breadcrumbPrefix} Found ${filteredDuplicateNewGeoEvents.length} new Geo Events`,
    'info',
  );
  ...
  if (filteredDuplicateNewGeoEvents.length > 0) {
    ...
    logger(
      `${breadcrumbPrefix} Created ${geoEventsCreatedCount} Geo Events`,
      'info',
    );
  }
