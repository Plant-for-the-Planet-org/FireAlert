...
function shouldContinueDeletion(
  startTime: number,
  type_of_cleanup: string = 'database',
): boolean {
  if (Date.now() - startTime > MAX_DURATION) {
    logger(
      `Db-Cleanup Approaching max duration. Exiting ${type_of_cleanup} cleanup early.`,
      'info',
    );
    return false;
  }
  ...
  while (continueDeletion) {
    const geoEventsToDelete = await prisma.geoEvent.findMany({
      where: {
        eventDate: {
          lt: new Date(
            currentDateTimeAt1AM.getTime() - 30 * 24 * 60 * 60 * 1000,
          ),
        },
      },
      take: batchSize,
...
async function deleteVerificationRequests() {
  const deletedVerificationRequests =
    await prisma.verificationRequest.deleteMany({
      where: {
        expires: {
          lt: new Date(
            currentDateTimeAt1AM.getTime() - 7 * 24 * 60 * 60 * 1000,
          ),
        },
      },
    });
  logger(
    `Deleted ${deletedVerificationRequests.count} expired verification requests`,
    'info',
  );
...
async function cleanUsers(startTime: number) {
  ...
  for (const user of usersToBeDeleted) {
    ...
    logger(
      `USER DELETED: Sent account deletion confirmation email to ${user.id}`,
      'info',
    );
  ...
  logger(
    `Deleted ${countUsers} users, ${countSites} sites, ${countAlertMethods} alertMethods, ${countSiteAlerts} siteAlerts, ${countNotifications} notifications`,
    'info',
  );
...
async function cleanSites(startTime: number) {
  ...
  const allSites_toBe_deleted_Ids = (
...
async function cleanAlertMethods() {
  const deletedAlertMethods = await prisma.alertMethod.deleteMany({
    where: {
      deletedAt: {
        lte: new Date(currentDateTimeAt1AM.getTime() - 7 * 24 * 60 * 60 * 1000),
      },
    },
  });
...
async function cleanNotifications(startTime: number) {
  // Use a local variable to adjust batch_size, default use batch of 1000 entries
  const batchSize = 1000;
  //   const batchSize = 10;
  // keep track of totalNotificationsDeleted
  ...
  while (continueDeletion) {
    ...
    const deleteCount = notificationsToDelete.length;
    if (deleteCount === 0) {
      break;
    }
    ...
    await prisma.$transaction(async prisma => {
      ...
      // Increment stats notifications_deleted with totalNotificationsDeleted.
      await updateOrCreateStats('notifications_deleted', deleteCount);
    });

    totalNotificationsDeleted += deleteCount;

    // Watchdog Timer
    continueDeletion = shouldContinueDeletion(startTime, 'Notification');
  }

  logger(`Deleted ${totalNotificationsDeleted} notifications`, 'info');
  return totalNotificationsDeleted;
}

// This cron will also help with GDPR compliance and data retention.
export default async function dbCleanup(
  req: NextApiRequest,
  res: NextApiResponse,
) {
    ...
    if (validCleanupOptions.includes(tableToClean)) {
      ...
      switch (tableToClean) {
        ...
        case 'verificationRequest': {
          const verificationRequestsDeleted =
            await deleteVerificationRequests();
        ...
        case 'notification': {
          const notificationsDeleted = await cleanNotifications(startTime);
          if (notificationsDeleted == 0) {
            isClean = true;
          }
          res.status(200).json({
            message: `Successfully deleted ${notificationsDeleted} notifications`,
            isClean: isClean,
            status: 200,
          });
          break;
