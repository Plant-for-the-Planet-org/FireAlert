...
export default async function alertFetcher(
  ...
  while (true) {
    ...
    logger(
      `Running Geo Event Fetcher. Taking ${activeProviders.length} eligible providers.`,
      'info',
    );
      ...
      return await geoEventProvider
        ...
        .then(async geoEvents => {
          ...
          if (geoEvents.length > 0) {
            eventCount = await processGeoEvents(
              breadcrumbPrefix,
  ...
  if (newSiteAlertCount > 0) {
    ...
    logger(
      `Added ${notificationCount} notifications for ${newSiteAlertCount} alerts`,
      'info',
    );
  } else {
    logger(
      `All done. ${newSiteAlertCount} Alerts. No new notifications. Waving Goodbye!`,
      'info',
    );
  }
