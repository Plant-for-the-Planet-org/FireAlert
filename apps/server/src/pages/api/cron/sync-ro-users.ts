...
export default async function syncROUsers(
  req: NextApiRequest,
  res: NextApiResponse,
) {
  ...
  const allProjectsPPWebApp: TreeProjectExtended[] =
    await fetchAllProjectsWithSites();
  ...
  const projectsPP = projectsPP1.concat(
    projectsPP2.filter(
      projectPP2 =>
  ...
  const ids_sitesThatAreOrWereOnceRemote = sitesThatAreOrWereOnceRemote.map(
    site => site.id,
  );
  const map_ids_sitesThatAreOrWereOnceRemote_to_remoteId = new Map();
  sitesThatAreOrWereOnceRemote.forEach(site => {
    map_ids_sitesThatAreOrWereOnceRemote_to_remoteId.set(
      site.remoteId,
      site.id,
    );
  });
    ...
    for (const projectPP of projectsInPP_not_in_FA) {
      ...
      if (sitesFromPP && sitesFromPP.length > 0) {
        for (const siteFromPP of sitesFromPP) {
          ...
          const siteId_mapped_from_remoteId =
            map_ids_sitesThatAreOrWereOnceRemote_to_remoteId.get(remoteId_PP);
          ...
          if (geometry && geometry.type) {
            ...
            if (
              ids_sitesThatAreOrWereOnceRemote.includes(
                siteId_mapped_from_remoteId,
              )
            ) {
            ...
            logger(
              `Skipping site with id ${remoteId_PP} due to null geometry or type.`,
              'info',
            );
          }
        }
  ...
  const remoteIdsList_SiteFA = sitesFA.map(
    siteFA => siteFA.remoteId,
  ) as string[];

  // Create a list of site IDs from PP
  const remoteIdsList_PP: string[] = [];
  for (const projectPP of projectsPP) {
    if (projectPP.sites && projectPP.sites.length > 0) {
      // Checking if sites is not null before accessing it
  ...
  const remoteIdsInFA_NotInPP = remoteIdsList_SiteFA.filter(
    remoteId => !remoteIdsList_PP.includes(remoteId),
  );

  const siteIdsInFA_NotInPP = sitesFA
    .filter(site => remoteIdsInFA_NotInPP.includes(site.remoteId as string))
    .map(site => site.id);
  ...
  try {
    ...
    if (siteIdsInFA_NotInPP.length > 0) {
      ...
      logger(
        `Soft Deleting ${siteIdsInFA_NotInPP.length} sites not present in the Webapp`,
        'info',
      );
    }
    ...
    for (const projectPP of projectsPP) {
      ...
      const projectLastUpdatedFormPP = moment(
        projectLastUpdated,
        'YYYY-MM-DD HH:mm:ss',
      ).toDate(); // Convert to Date object
      const projectFromDatabase = newProjectsFA.find(
        project => project.id === projectId,
      );

      // If the project has been updated in the PP API, update the project and its sites in the database
      if (
        projectFromDatabase!.lastUpdated?.getTime() !==
        projectLastUpdatedFormPP.getTime()
      ) {
        updatePromises.push(
      ...
      if (sitesFromPPProject && sitesFromPPProject.length > 0) {
        for (const siteFromPP of sitesFromPPProject) {
          ...
          if (geometry && geometry.type) {
            const siteId_in_FADatabase =
              map_ids_sitesThatAreOrWereOnceRemote_to_remoteId.get(
                remoteId_fromPP,
              );
            const radius = 0;
            ...
            if (siteId_in_FADatabase) {
              siteFromDatabase = sitesFA.find(
                site => site.id === siteId_in_FADatabase,
              );
            }
            ...
            if (!siteFromDatabase) {
              ...
              siteFromDatabase.lastUpdated?.getTime() !==
              siteLastUpdatedFromPP.getTime()
            ) {
              updatePromises.push(
                prisma.site.update({
                  where: {
            ...
            // Handle the case where geometry or type is null
            logger(
              `Skipping site with id ${remoteId_fromPP} due to null geometry or type.`,
              'info',
            );
          }
        }
    ...
    logger(
      `Created ${createCount} items. Updated ${updateCount} items. Deleted ${deleteCount} items.`,
      'info',
    );
