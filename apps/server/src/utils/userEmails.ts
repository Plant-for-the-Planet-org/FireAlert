...
const sendWelcomeEmail = async (user: User): Promise<boolean> => {
  const params: NotificationParameters = {
    message: `<p>Dear ${getName(user)},</p>
      ...
      <p>Welcome to FireAlert, an innovative, open-source application designed to make fire monitoring a seamless experience!</p>
      
      <p>As a FireAlert user, you have the capability to create new sites and closely monitor them for any fire incidents. 
      For those who have an account with the Plant-for-the-Planet platform previously, we have already imported your sites into FireAlert for your convenience. 
...
const sendAccountDeletionConfirmationEmail = async (user: User): Promise<boolean> => {
  const params: NotificationParameters = {
    message: `<p>Dear ${getName(user)},</p>
      ...
      <p>We're writing to confirm that your FireAlert account has been successfully deleted.</p>
...
const sendEmailVerificationCode = async (user: User, verificationEmail: string, verificationCode: string): Promise<boolean> => {
  const params: NotificationParameters = {
    message: `<p>Dear ${getName(user)},</p>
      ...
      <p>To ensure the security and authenticity of your account, we require a one-time verification of your email address.</p>
      
      <p>Your FireAlert account verification code is <b>${verificationCode}</b>.This code will expire in 30 minutes.</p>
      ...
      <p>Thank you for using FireAlert, and we look forward to assisting you in your fire monitoring efforts.</p>
