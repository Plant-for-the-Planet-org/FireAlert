...
export const alertMethodRouter = createTRPCRouter({
    ...
    .mutation(async ({ctx, input}) => {
      ...
      try {
        ...
        if (alertMethod.isVerified) {
          ...
          const sendVerification = await handlePendingVerification(
            ctx,
            alertMethod,
          );
  ...
  createAlertMethod: protectedProcedure
    ...
    .mutation(async ({ctx, input}) => {
      ...
      if (existingAlertMethod?.deletedAt) {
        ...
        await limitAlertMethodBasedOnPlan({
          ctx,
          userId,
          userPlan: userPlan,
          method: input.method,
      ...
      if (input.method === 'device') {
        ...
        if (isDeviceVerified) {
          ...
          return {
            status: 'success',
            message:
              'Device has been Verified. Successfully create alertMethod.',
            data: alertMethod,
          };
        } else {
          throw new TRPCError({
      ...
      try {
        ...
        return {
          status: 'success',
          message:
            'Successfully Created AlertMethod. ' + sendVerification.message,
  ...
  getAlertMethods: protectedProcedure.query(async ({ctx}) => {
    const userId = ctx.user!.id;
    try {
  ...
  getAlertMethod: protectedProcedure
    ...
    .query(async ({ctx, input}) => {
      const userId = ctx.user!.id;
      const existingAlertMethod = await checkUserHasAlertMethodPermission({
        ctx,
        alertMethodId: input.alertMethodId,
        userId: userId,
      });
      if (existingAlertMethod.deletedAt) {
        throw new TRPCError({
  ...
  updateAlertMethod: protectedProcedure
    ...
    .mutation(async ({ctx, input}) => {
      ...
      const existingAlertMethod = await checkUserHasAlertMethodPermission({
        ctx,
        alertMethodId: input.params.alertMethodId,
        userId: userId,
      });
      if (existingAlertMethod.deletedAt) {
        throw new TRPCError({
