# !/bin/bash
#
# ProtectedPlanet PostGIS Integration & FireAlert Sync
#
# This script automates the process of:
# 1. Checking for required PostGIS tools
# 2. Downloading and preparing ProtectedPlanet dataset
...
# Default values
# DATASET_URL="https://d1gam3xoknrgr2.cloudfront.net/current/WDPA_WDOECM_Apr2025_Public_all_shp.zip"
DATASET_URL="https://d1gam3xoknrgr2.cloudfront.net/current/WDPA_WDOECM_Apr2025_Public_AF_shp.zip"
ENVIRONMENT="production"
TEMP_DIR="$(pwd)/tmp"
OUTPUT_SQL="${TEMP_DIR}/ProtectedPlanet.sql"
BACKUP_PATH=""
IS_PRODUCTION=true
# ZIP_PATH="${TEMP_DIR}/protected_planet.zip"
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      --help)
        ...
        echo "Options:"
        echo "  --url URL     URL to the ProtectedPlanet dataset zip file"
...
convert_shapefiles_to_sql() {
  ...
  while IFS= read -r polygon_file; do
    ...
    shp2pgsql -I -s 4326 -g geom -p "$polygon_file" "protected_areas" > "${TEMP_DIR}/temp_schema.sql" 2>/dev/null
    cat "${TEMP_DIR}/temp_schema.sql" >> "$schema_polygons"
    
    # Data-only conversion
    shp2pgsql -I -s 4326 -g geom -a "$polygon_file" "protected_areas" > "${TEMP_DIR}/temp_data.sql" 2>/dev/null
...
get_firealert_db_url() {
  ...
  # In a real script, you might get this from environment variables
...
# Main function
main() {
  print_message info "Starting ProtectedPlanet PostGIS Integration & FireAlert Sync"
  ...
  if [[ $import_success -eq 0 ]]; then
    print_message success "ProtectedPlanet integration completed successfully"
    return 0
  else
    print_message error "ProtectedPlanet integration failed"
    return 1
  fi
}
